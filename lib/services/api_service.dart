...
class ApiService {
  ...
  static const String _apiKey = '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';
  ...
  static Future<List<String>> generateInterviewQuestions({
    ...
    try {
      ...
      final prompt = """Generate $numberOfQuestions interview questions for a $difficulty level $field developer 
      ...
      final response = await http.post(
        ...
        headers: {
          ...
          'HTTP-Referer': 'https://interview-service.app', // Replace with your app domain
        },
        body: jsonEncode({
          'model': _defaultModel,
          'messages': [
            {
              'role': 'system',
              'content': 'You are an expert technical interviewer for $field positions.'
            },
            {
              'role': 'user',
              'content': prompt
  ...
  static Future<Map> evaluateAnswer({
    ...
    try {
      ...
      final prompt = """Evaluate the following interview answer for a $difficulty level $field developer 
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse.containsKey('choices') && 
          ...
          try {
            ...
            // Parse the content as JSON
            final dynamic parsedContent = jsonDecode(content);
            print('Parsed evaluation content type: ${parsedContent.runtimeType}');
            
            // Ensure we have a valid response with score and feedback
            if (parsedContent is Map) {
              // Check if it has the expected fields
              if (parsedContent.containsKey('score') && parsedContent.containsKey('feedback')) {
                ...
                parsedContent.forEach((key, value) {
                  if (value is num && (key.toLowerCase().contains('score') || key == 'rating')) {
                    result['score'] = value;
                ...
                if (result.containsKey('score') && result.containsKey('feedback')) {
                  return result;
                }
              }
            }
            ...
            return {
              'score': 0,
              'feedback': 'Error: Could not parse evaluation response. Please try again.'
            };
          } catch (e) {
